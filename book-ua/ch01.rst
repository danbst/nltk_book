...
========================================
1. Обробка природної мови разом з Python
========================================

Нам легко оперувати текстами з тисячами слів. А що ми можемо з ними робити, 
якщо ми тільки можемо писати певні простенькі програми? В даному розділі 
ми обговоримо наступні питання:

#. Що можна отримати, об'єднавши прості програмні технології з великою кількістю текстів?
#. Як можна автоматично витягувати ключові слова і фрази, по котрим можна сформувати стиль і зміст тексту?
#. Які інструменти і техніки нам дає Python для роботи такого типу?
#. Які задачі обробки природних мов є найцікавішіми?

Даний розділ розділено на частини двох досить різних типів, які досить сильно 
відрізняються стилем мовлення. В частинах "обробка мови" ми візьмемось за деякі 
лінгвістичні програмні задачі без пояснень, як вони працюють. В частинах 
"ближче до Python" ми будемо час від часу переглядати деякі ключові концепти 
програмування. Тип частини ми будемо показувати в заголовку, але в подальших 
розділах вони будуть перемежовуватись. Сподіваємось, такий вступний стиль 
допоможе вам зрозуміти, що вас чекає в подальшому, і разом з тим охопити широкий 
спектр елементарних принципів у лінгвістиці та інформатиці. Якщо у вас є базові 
навички у обох галузях, ви можете перейти одразу до sec-automatic-natural-language-understanding_; 
ми будемо повторювати будь-які важливі поняття в наступних розділах, і якщо ви 
щось пропустили, ви легко можете звернутись до онлайн матеріалів на |NLTK-URL|. 
Якщо даний матеріал для вас новий, то даний розділ дасть більше питань, ніж 
відповідей, питань, котрі освітлено в наступних розділах.

.. _sec-computing-with-language-texts-and-words:

---------------------------------
Комп'ютери і мова: тексти і слова
---------------------------------

Всі ми знайомі з поняттям "текст", оскільки кожен день ми його читаємо і пишемо. 
Зараз ми будемо інтепретувати текст як "чисті дані" для наших програм, які будуть 
оброблювати і аналізувати його у багатьох цікавих способах. Але перед цим треба 
зупстити інтепретатор Python.

Знайомство з Python
-------------------

Одною з приємних властивостей Пайтона є те, що він дозволяє вводити код у 
інтерактивний `інтепретатор`:dt: |mdash| програму, яка буде виконувати ваші 
програми. Ви можете запустити інтепретатор, використовуючи простий графічних 
інтерфейс |IDLE|. Під Mac ви знайдете його у *Applications*\ |rarr|\ *MacPython*,
а під Windows у *All Programs*\ |rarr|\ *Python*. Під Unix ви можете просто 
запустити з командного рядка програму ``idle`` (або ``python``, якщо ``idle`` не 
встановлено). Інтерпретатор надрукує текст про поточну версію Пайтона, просто 
перевірте, що у вас запущено інтерпретатор версії 2.4 або 2.5.
...
.. note::
   Якщо вам не вдалося запустити інтепретатор, мабуть він у вас просто не 
   встановлений. В таком разі завітайте на сайт |PYTHON-URL| для 
   інструкцій по встановленню.

Вказівка до вводу ``>>>`` показує, що інтепретатор запущено і він очікує ввід 
коду або даних. Якщо ви копіюєте приклади з книги, то не вводьте "``>>>``", до 
програми воно не відноситься. Давайте спробуємо використиа Пайтон як калькулятор:

    >>> 1 + 5 * 2 - 3
    8
    >>>

Як тільки інтепретатор закінчив обчислення і їх вивід на дисплей, вказівка до 
вводу з'являється знову. Це означає, що Пайтон очікує наступні інструкції.

.. note:: |TRY|
   Спробуйте ввести свої вирази. Можете використовувати зірочку (``*``) для 
   множення, слеш (``/``) для ділення і дужки для групуваннята пріоритетів у 
   обчисленні. Зауважте, що ділення працює не так як ви очікували |mdash| вона 
   виконує цілочисленне ділення (з урізанням дробової частини) коли ви вводите 
   ``1/3``, і дробове ділення (з плаваючою точкою) коли ви вводите ``1.0/3.0``. 
   Щоб використовувати більш очікувану поведінку для ділення, вам потрібно 
   ввести ``from __future__ import division``
...
Попередній приклад показує, як ви можете інтерактивно працювати з Пайтоном, 
експериментувати з різними виразами у мові і переглядати результати 
експериментів. Давайте тепер спробуємо ввести беззмістовний вираз, щоб 
поглянути як інтепретатор обробляє помилки:
...
Код згенерував `синтаксичну помилку`:dt:. В Пайтоні знак плюса в кінці 
інструкції немає змісту. Інтерпретатор показує рядок, де проблема виникла 
(рядок 1 ``<stdin>``, що означає "standart input" - "стандартний ввід").

Тепер, коли ви можете використовувти інтепретатор Пайтона, давайте почнемо 
працювати з мовними даними.

Початок роботи з NLTK
---------------------

Перед тим як йти далі, потрібно встановити |NLTK|\ , котрий можна вільно 
завантажити з |NLTK-URL|. Дотримуйтесь інстуркцій на сайті для завантаження 
версії, що відповідає вашій системі.

Коли ви завантажили |NLTK|, запустіть інтерпретатор Пайтона як і раніше і 
встановіть дані, потрібні для книги просто ввівши наступні дві команди у 
полі вводу Пайтона, а потім вибравши колекцію ``book``, як показано на 
fig-nltk-downloader_.
...
.. figure:: ../images/nltk-downloader.png
   :scale: 100:100:100
   Завантаження колекції книг NLTK: переглянути доступні пакунки можна за через 
   ``nltk.download()``. Закладка **Collections** (колекції) у завантажувача 
   показує як пакунки групуються у набори і вам потрібно вибрати рядок, 
   помічений як **book** (книга), щоб отримати усю інформацію про необхідні 
   дані для прикладів з даної книги. Він складається з близько 30 заархівованих 
   файлів, що займають близько 100 Мбайт місця на диску. Повний набір даних 
   (тобто, пакунок **all** у завантажувача) десь у 5 разів більший (на момент 
   написання) і продовжує збільшуватись.

Як тільки дані завантажено на комп'ютер, ви можете підгрузити їх частину за 
допомгою інтепретатора Пайтон. Першим кроком введіть спеціальну команду, яка 
вкаже інтепретатору, що потрібно підгрузити деякі тести для дослідження: 
``from nltk.book import *``. ВОно каже "з модуля ``book``, підтягни усі 
елементи". Модуль ``book`` містить усю інформацію, яка вам знадобиться при 
читанні даного розділу. Після виводу вступного повідомлення, будуть підвантажені 
тексти кількох книг (це займе кілька секунд). Ось знову команди, разом з 
виводом, який ви побачите. Будьте уважні, передруковуючи текст і не зубвайте, 
що ``>>>`` вводити не потрібно.
...
Кожен раз, коли нам знадобляться ці тексти, ми просто введемо їх назви у рядок 
вводу Пайтона:
...
Тепер, коли ми можемо використовувати інтепретатор Пайтона, і завантажили певні 
дані, можемо приступати до роботи.

Пошук тексту
------------

Існує багато способів аналізу змісту тексту, окрім простого читання. 
Зпівставлене відображення показує нам кожну зустріч вказаного слова разом з 
частиною контексту. Для прикладу, ми шукаємо слово `monstrous`:lx: у *Мобі Діку* 
і для цього вводимо ``text1``, потім крапку, потім слово ``concordance`` і на 
сам кінець слово ``"monstrous"`` в дужках:
...
.. note:: |TRY|
  Спробуйте пошукати інші слова; аби зменшити час перенабирання, використовуйте 
  клавішу "up-arrow" (стрілочка вверх), комбінацію "Ctrl-up-arrow" або "Alt-P" 
  для використання попередніх введених команд, і потім змінюйте потрібне слово. 
  Ви також можете спробувати шукати слова у інших текстах, які ми підвантажили 
  раніше. Наприклад, пошукайте у *Sense and Sensibility* ("Розум і почуття", 
  роман Джейн Остін) слово `affection`:lx: (прихильність) за допомогою команди 
  ``text2.concordance("affection")``. Пошукайте у книзі Буття 
  ("Book of Genesis") як довго жили різні люди, використовуючи команду 
  ``text3.concordance("lived")``. Також можете глянути на ``text4``, 
  *Inaugural Address Corpus* (Зібрання іннагураційних звернень), щоб проглянути 
  приклади англійської мови аж до 1789 року і пошукати слова типу `nation`:lx:, 
  `terror`:lx:, `god`:lx:; примітьте, що у різні роки слова вживались у різних 
  контекстах. Ми також включили ``text5``, *NPS Chat Corpus*: пошукайте у ньому 
  нешаблонні слова, як от `im`:lx:, `ur`:lx:, `lol`:lx:.
  (Зауважте, що ``text5`` містить нецензурні вирази!)

Якщо ви витратили трохи часу на вивчення прикладі, то сподіваємось, у вас 
з'явилось відчуття багатогранності і розмаїтості мови. У наступному розділі ви 
вивчите як отримувати доступ до більшої кількості текстів, включаючи 
не-англійські твори.

Зпівставлення допомагає нам бачити слова в контексті. Наприклад, ми побачили, 
що слово `monstrous`:lx з'являлось у таких контекстах, як `the ___ pictures`:lx:
and `the ___ size`:lx:. Які інші слова з'являжються у схожих контекстах? Ми 
можемо взнати це за допомогою функції ``similar``:
...
Зауважте, ми отримуємо різні результати для різних текстів. Джейн Остін вживає 
це слово дещо у іншому значенні, ніж Герман Мелвіл; для неї слово 
`monstrous`:lx: має позитивний контекст і інколи слугує як синонім до слова 
`very`:lx: (дуже).

Функція ``common_contexts`` дозволяє нам вивчати тільки контексти, які 
суміщують два або більше слів, наприклад `monstrous`:lx: і `very`:lx:. Нам 
потрібно взяти ці слова у квадратні дужки і звичайні дужки, розділивши комами:
...
.. note:: |TRY|
   Виберіть іншу пару слів і порівняйте їх використання у двох різних текстах, 
   використовуючи функції ``similar()`` і ``common_contexts()``.

Це одна з задач |mdash| знайти конретне слово в тексті і вивести інші слова, 
які зустрічаються у схожому контексті. Однак, ми також можемо визначити 
*позицію* слова у тексті: яке це по номеру слово в тексті. Інформацію про 
розміщення можна вивести на екран за допомогою `графіку дисперсії`:dt:. Кожна 
смужка означає одну зустріч слова, кожен ряд означає цілий текст. 
На fig-inaugural_ ми бачимо вражаючі візерунки використання слів за останні 
220 років (у тексті, що було утворено з'єднаннями виступів на інагураціях). 
Ви можете відтворити графік так, як це показано нижче. Ви можете спробувати 
інші слова (наприклад, `liberty`:lx: |mdash| свобода, `constitution`:lx: 
|mdash| конституція) або інші тексти. Чи можете ви передбачити дисперсію слова, 
яке ви знаєте? Як і раніше, беріть до уваги лапки, коми і дужки.
...
.. figure:: ../images/inaugural.png
   ...
   Графік лексичної дисперсії для слів з інагураційних промов президентів США. 
   Цей текст може використовуватись для відслідковування змін у мові на протязі 
   тривалого часу.

.. note:: |IMPORTANT|
   Вам необхідно встановити пакунки Пайтона ``NumPy`` і ``Matplotlib``, щоб 
   мати змогу відтворити графіки з даної книги. Завітайте на |NLTK-URL|, щоб 
   дістати інструкції по встановленню.

Давайте тепер спробуємо згенерувати якийсь випадковий текст у одному з стилів, 
які ми уже бачили. Щоб це зробити, ми вводимо ім'я тексту і викликаємо 
функцію ``generate`` (ми також додаємо дужки, але в дужках нічого писати не 
потрібно).
...
Зауважте, що перший раз команда виконується довго, тому що вона збирає 
статистику про послідовності слів. Кожен раз, коли ви її запускаєте, ви 
отримаєте різний вивід. Тепер спробуйте згенерувати випадковий текст у стилі 
інагураційної промови або інтернет-чату. І хоча текст є випадковим, він 
використовує слова і фрази з базового тексту, зарахунок чого вдається відтворити 
базовий стиль і контекст. (А чого на вашу думку не вистачає у випадково 
згенерованому тексті?)

.. note::
   Коли ``generate`` виводить результат, пунктуація відділяється від 
   передуючого слова. Хоча це і не є коректним для тексту англійською, ми це 
   робимо щоб показати незалежність слів і пунктуації. Детальініше про це 
   ви дізнаєтесь у chap-words_.

Створення словника
------------------

Найбільш очевидним фактом щодо використаних вище текстів є те, що вони відрізняються 
використовуваним словником. У даній частині ми побачимо, як використовувати комп'ютер 
для підрахунку кількості слів, причому кількома різними способами.
Як і раніше, ви швидко почнете самостійно експериментувати з інтерпретатором 
Пайтона, навіть якщо ви досі його не вивчали систематично. Спробуйте змінювати
приклади і виконуйте завдання у кінці розділу, щоб перевірити своє розуміння Пайтона.

Давайте почнемо з з'ясування довжини тексту від початку до кінця,
в словах та знаках пунктуації, які з'являються.  Ми використовуємо
термін ``len`` для отримання довжини чогось, що ми будемо застосовувати тут для Книги Буття:
...
#. |easy| Спробуйте використати Python як інтерактивний калькулятор, вводячи
   вирази вигляду ``12 / (4 + 1)``.
   
#. |easy| Якщо ми маємо алфавіт з 26 літер, то всього ми можемо утворити
   26 у 10 степені (або ``26 ** 10``) десятисимвольних рядків. Отже... ми 
   можемо утворити ``141167095653376L`` рядків (``L`` в кінці числа у Python 
   показує, що число відноситься до типу long). Підрахуйте, скільки можна 
   утворити 100-символьних рядків?

#. |easy| У Пайтоні можна застосовувати "множення" до списків. Що станеться,
   якщо ви напишите ``['Monty', 'Python'] * 20`` або ``3 * sent1``?
